...
<main>
  ...
  <h2>
    <a href="notice.php" title="공지사항">공지사항</a>
  </h2>
  <div class="notice_box">
  <ul class="notice_ul">
    <form name="notice" id="notice_form" method="get" action="notice.php">
      <?php
      $sql_notice = "select * from board_notice ".($category != '' ? "WHERE $category LIKE '%$search%' " : "")." order by notice_id desc;";
      $result_notice= mysqli_query($con, $sql_notice);
        ...
        $sql = "select * from board_notice ".($category != '' ? "WHERE $category LIKE '%$search%' " : "")." order by notice_id desc limit $start, $list_num"; /* paging : 쿼리 작성 - limit 몇번부터, 몇개 */
